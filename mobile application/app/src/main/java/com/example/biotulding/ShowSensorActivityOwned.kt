...
class ShowSensorActivityOwned: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val imageView = findViewById<ImageView>(R.id.imageView_sensor)
        ...
        val btn = findViewById<FloatingActionButton>(R.id.circularButton)
        ...
        val statistic_btn = findViewById<FloatingActionButton>(R.id.circularButton_thingspeak)
        statistic_btn.setOnClickListener{
            GlobalScope.launch(Dispatchers.IO) {
                val intent2 = Intent(this@ShowSensorActivityOwned, ThingSpeakActivity::class.java)
                try {
                    val response =
                        fetchData("$resource_url/building/getTSlink?building_id=$building_id&room_id=$roomid")
                    url_thingspeak = response
                    intent2.putExtra("url",url_thingspeak)
                    println("Link:$url_thingspeak")
                } catch (e: IOException) {
                    println("Error")
                }
